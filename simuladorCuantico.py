...
def operarClicksSistemaC(rendijas, blancos, clicks):
    ...
    if clicks == 0:
        ...
        for i in range(len(vectorCol)):
            for j in range(1):
                vectorResultado.append(moduloComplejos(vectorCol[i][j])**2)
...
def moculoCuadrado(listaTuplas):
    resultado = []
    for i in range(len(listaTuplas)):
        resultado.append(moduloComplejos(listaTuplas[i]) ** 2)
    return resultado
...
def main():
    tupla = operarClicksSistemaC(2, 5, 1)
    graficarEstados(tupla[1])
